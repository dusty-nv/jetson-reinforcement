...
resize = T.Compose([T.ToPILImage(),
                    T.Resize(args.resolution, interpolation=Image.CUBIC),
                    T.ToTensor()])
...
#plt.show()
#cv2.imshow('Example extracted screen', cv2.cvtColor(get_screen().cpu().squeeze(0).permute(1, 2, 0).numpy(), cv2.COLOR_RGB2BGR))  
#cv2.waitKey(1) 
...
for i_episode in range(args.train_episodes):
    ...
    for t in count():
        ...
        cv2.imshow('DQN screen', cv2.cvtColor(current_screen.cpu().squeeze(0).permute(1, 2, 0).numpy(), cv2.COLOR_RGB2BGR))  
        cv2.waitKey(1) 
        ...
        if done:
            ...
            print(f'episode {i_episode}  steps {steps}  duration {t}  reward {ep_reward}')
            #plot_durations()
            break
